...
def get_feature():
    ...
    dataframe = efu.get_turn_number(dataframe)
    print(dataframe.head())

    # 加入天气相关信息，
    dataframe = efu.get_weather_info(dataframe)
    print(dataframe.head())
    
    # 加入节日信息
    dataframe = efu.get_holiday(dataframe)
    print(dataframe.head())
    
    # 加入变换后的坐标的信息，适用pca来变换坐标
    dataframe = efu.get_pca(dataframe)
    print(dataframe.head())
    ...
    dataframe = efu.ex_speed_hvsn(dataframe)
    dataframe = efu.ex_speed_manhtn(dataframe)
    print(dataframe.shape)
